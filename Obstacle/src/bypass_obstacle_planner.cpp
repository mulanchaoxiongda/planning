...
bool PiecewiseJerkPathOptimization::ExpandObstacles() {
    ...
    for (int i = 0; i < nums_points; ++i) {
        if (fesiable_region_[i].l_max < modified_road_half_wid ||
            fesiable_region_[i].l_min > -modified_road_half_wid) {
            obstacle_sign[i] = 1;
        }
    }

    int left = 0, right = 0;
    double lmin_max = -modified_road_half_wid, lmax_min = modified_road_half_wid;
    ...
    while (right < nums_points) {
        if (occupy_sign) {
            if (obstacle_sign[right]) {
                ...
                lmin_max = -modified_road_half_wid;
                lmax_min = modified_road_half_wid;
            }
        } else {
            if (obstacle_sign[right]) {
                occupy_sign = true;
    ...
    for (auto member : expand_intervals_) {
        if(ExpandObstacle(member, modified_road_half_wid)) {
           return true;
        }
    }
...
bool PiecewiseJerkPathOptimization::ExpandObstacle(
    const ExpandInterval expand_interval, const double& modified_road_half_wid) {
    double map_error = 0.3; // todo : 
    double s_planning_margin = robot_len_ * 0.5 + 0.3; // todo : 调试
    double l_planning_margin = robot_wid_ * 0.5 + 0.2;
    double s_expand = map_error + s_planning_margin;
    double l_expand = map_error + l_planning_margin;

    int nums_points = fesiable_region_.size();
    int idx_expend = (int)(s_expand / fesiable_region_[1].s - fesiable_region_[0].s) + 1;
    double l_bound = robot_wid_ * 3.0; // 车体中心虚拟可通行道路半宽值
    double l_bound_obs = modified_road_half_wid - 0.001; // 虚拟可通行道路边缘外无障碍物判判据
    double min_fesiable_wid = robot_wid_ + 0.4; // todo : 调试

    int idx_start, idx_end;
    double lmin_max, lmax_min;

    idx_start = max(expand_interval.idx_start - idx_expend, 0);
    idx_end   = min(expand_interval.idx_end + idx_expend, nums_points - 1);

    lmin_max  = expand_interval.lmin_max > -l_bound_obs ? expand_interval.lmin_max + l_expand : expand_interval.lmin_max;
    lmin_max  = max(lmin_max, -l_bound);
    
    lmax_min  = expand_interval.lmax_min <  l_bound_obs ? expand_interval.lmax_min - l_expand : expand_interval.lmax_min;
...
bool PiecewiseJerkPathOptimization::CalPointFesiableRegion(
    ...
    double road_half_wid = robot_wid_ * 4.0 + road_wid_margin; // todo : 虚拟车道宽度
    ...
    vector<bool> l_occupy(nums_l_occupy, true);
    ...
    for (int i = 0; i < nums_l_occupy; ++i) { // 在global_path_基准点切线的垂线段上以栅格地图分辨率采点，并判断占用情况
        ...
        if (IsPointInMap(sample_x, sample_y, robot_pose_.x, robot_pose_.y)) { // todo : 根据NaviCostMap类的栅格地图数据细节 && IsInMap()函数细节，决定是否需要重写函数
            ...
            l_occupy[i] = false; // [说明] : 暂定超出局部障碍栅格图范围的点为自由点
        }
    ...
    while (right < nums_l_occupy) { // 搜索s-l系下最宽可通行路宽对应的边界[lmin, lmax]
        if (occupy_state) {
            if (l_occupy[right]) {
                ++right;
                continue;
    ...
    double wid_margin = 1.0, road_wid_min = robot_wid_ + wid_margin; // todo : 最窄可通行路宽----障碍膨胀前

...
while ~feof(fid)
    ...
    id_MOTOR = strfind(t1, '[obs_info]');
    if ~isempty(id_MOTOR)
        data = textscan(t1 , ObsInfo_Ide);
        ObsInfo(ObsInfo_Index, (1 : 4)) = data(1, (4 : 7));
        ObsInfo_Index = ObsInfo_Index + 1;
    end
end
...
plot(  cell2mat(RefPoi(:, 2)), cell2mat(RefPoi(:, 4)), 'green', ...        %% 近似局部轨迹
         cell2mat(StaRob(:, 2)), cell2mat(StaRob(:, 4)), 'b', ...
         cell2mat(GoalState(:, 2)), cell2mat(GoalState(:, 4)), 'blacko', ...
         cell2mat(ObsInfo(:, 2)), cell2mat(ObsInfo(:, 4)), 'red*'); grid on;
xlabel('横向位置(米)'); ylabel('纵向位置(米)'); set(gca, 'FontSize', 16); axis equal;
title('运动轨迹'); legend('规划', '反馈', '托盘位置','最近障碍物');
